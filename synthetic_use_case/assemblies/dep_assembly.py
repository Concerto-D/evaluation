...
class DepAssembly(Assembly):
    ...
    def __init__(self, p, reconf_config_dict, sleep_when_blocked=True, timeout=False):
        Assembly.__init__(self, f"dep_assembly_{p}", self.components_types, self.remote_component_names,
                          self.remote_assemblies_names, reconf_config_dict, sleep_when_blocked, timeout)
