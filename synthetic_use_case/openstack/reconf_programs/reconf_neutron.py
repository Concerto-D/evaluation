...
from synthetic_use_case.openstack.assemblies.neutron_assembly import NeutronAssembly


@create_timestamp_metric(TimestampType.TimestampEvent.DEPLOY)
def deploy(sc: Assembly, scaling_num: int):
    sc.add_component(f"neutron{scaling_num}", "Neutron")
    sc.connect(f"neutron{scaling_num}", "worker_service", f"mariadbworker{scaling_num}", "service")
    sc.connect(f"neutron{scaling_num}", "keystone_service", f"keystone{scaling_num}", "service")
    sc.push_b(f"neutron{scaling_num}", "deploy")
    sc.wait(f"neutron{scaling_num}")


@create_timestamp_metric(TimestampType.TimestampEvent.UPDATE)
def update(sc, scaling_num: int):
    sc.push_b(f"neutron{scaling_num}", "uninstall")
...
def execute_reconf(config_dict, duration, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num):
    sc = NeutronAssembly(config_dict, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num)
    sc.set_verbosity(2)
