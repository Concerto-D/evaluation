...
from synthetic_use_case.openstack.assemblies.nova_assembly import NovaAssembly


@create_timestamp_metric(TimestampType.TimestampEvent.DEPLOY)
def deploy(sc: Assembly, scaling_num: int):
    sc.add_component(f"nova{scaling_num}", "Nova")
    sc.connect(f"nova{scaling_num}", "mariadbworker_service", f"mariadbworker{scaling_num}", "service")
    sc.connect(f"nova{scaling_num}", "keystone_service", f"keystone{scaling_num}", "service")
    sc.push_b(f"nova{scaling_num}", "deploy")
    sc.wait(f"nova{scaling_num}")


@create_timestamp_metric(TimestampType.TimestampEvent.UPDATE)
def update(sc, scaling_num: int):
    sc.push_b(f"nova{scaling_num}", "interrupt")
    sc.wait(f"mariadbworker{scaling_num}", wait_for_refusing_provide=True)
    sc.push_b(f"nova{scaling_num}", "update")  # TODO: renamed in uninstalled (trans adapted but not behavior)
    sc.wait(f"keystone{scaling_num}", wait_for_refusing_provide=True)
    sc.push_b(f"nova{scaling_num}", "deploy")
    sc.wait(f"nova{scaling_num}")
...
def execute_reconf(config_dict, duration, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num):
    sc = NovaAssembly(config_dict, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num)
    sc.set_verbosity(2)
