...
from synthetic_use_case.openstack.assemblies.glance_assembly import GlanceAssembly


@create_timestamp_metric(TimestampType.TimestampEvent.DEPLOY)
def deploy(sc: Assembly, scaling_num: int):
    sc.add_component(f"glance{scaling_num}", "Glance")
    sc.connect(f"glance{scaling_num}", "worker_service", f"mariadbworker{scaling_num}", "service")
    sc.connect(f"glance{scaling_num}", "keystone_service", f"keystone{scaling_num}", "service")
    sc.push_b(f"glance{scaling_num}", "deploy")
    sc.wait(f"glance{scaling_num}")


@create_timestamp_metric(TimestampType.TimestampEvent.UPDATE)
def update(sc, scaling_num: int):
    sc.push_b(f"glance{scaling_num}", "uninstall")
...
def execute_reconf(config_dict, duration, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num):
    sc = GlanceAssembly(config_dict, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num)
    sc.set_verbosity(2)
