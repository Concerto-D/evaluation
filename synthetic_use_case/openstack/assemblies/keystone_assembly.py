...
from synthetic_use_case.openstack.assemblies.keystone import Keystone


class KeystoneAssembly(Assembly):
    def __init__(self, reconf_config_dict, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num):
        ...
        remote_assemblies = [f"mariadbworker{scaling_num}", f"glance{scaling_num}", f"nova{scaling_num}", f"neutron{scaling_num}"]
