...
from synthetic_use_case.str_cps.assemblies.sensor_assembly import SensorAssembly


@create_timestamp_metric(TimestampType.TimestampEvent.DEPLOY)
def deploy(sc: Assembly, scaling_num: int):
    sc.add_component(f"sensor{scaling_num}", "Sensor")
    sc.connect(f"sensor{scaling_num}", "sensor_config_use", f"listener{scaling_num}", "listener_config_provide")
    sc.connect(f"sensor{scaling_num}", "sensor_service_use", f"listener{scaling_num}", "listener_service_provide")
    sc.push_b(f"sensor{scaling_num}", "deploy")
    sc.wait(f"sensor{scaling_num}")


@create_timestamp_metric(TimestampType.TimestampEvent.UPDATE)
def update(sc, scaling_num: int):
    sc.push_b(f"sensor{scaling_num}", "pause")
    sc.wait(f"listener{scaling_num}", wait_for_refusing_provide=True)
    sc.push_b(f"sensor{scaling_num}", "deploy")
    sc.wait(f"sensor{scaling_num}")
...
def execute_reconf(config_dict, duration, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num):
    sc = SensorAssembly(config_dict, waiting_rate, version_concerto_d, reconfiguration_name, nb_concerto_nodes, scaling_num)
    sc.set_verbosity(2)
