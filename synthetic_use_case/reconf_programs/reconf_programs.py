...
def get_assembly_parameters(args) -> Tuple[Dict, float, bool, Optional[str], str, Optional[int]]:
    ...
    dep_num = int(args[6]) if len(args) > 6 else None
    return loaded_config, uptime_duration, waiting_rate, timestamp_log_dir, execution_expe_dir, dep_num


def initialize_reconfiguration():
    config_dict, duration, waiting_rate, timestamp_log_dir, execution_expe_dir, dep_num = get_assembly_parameters(sys.argv)
    ...
    return config_dict, duration, waiting_rate, dep_num